...
export class SyncClient extends EventEmitter {
  ...
  constructor(config: SyncClientConfig) {
    ...
    this.debouncedAddFile = this.debounce(this.processFileAddQueue.bind(this), 500);
    this.debouncedAddDir = this.debounce(this.processDirAddQueue.bind(this), 500);
  ...
  private validateConfig(): void {
    ...
    if (!this.config.apiBaseUrl) {
      console.warn('⚠️  Warning: apiBaseUrl is not set. Syncing to cloud will not work.');
    }
  ...
  async start(): Promise<void> {
    ...
    this.watcher.on('ready', async () => {
      progress.log(`✅ Initial scan complete via Watcher. Reconciling ${initialScanFiles.length} files and ${initialScanDirs.length} directories...`);
      ...
      progress.log('✅ Reconciliation complete. Sync client fully operational.');
    });
    ...
    this.watcher.on('file:rename', async (oldPath: string, newPath: string, stats: any) => {
      ...
      try {
        if (this.dbManager) {
          ...
          let oldRelPath = normalizedOldPath.substring(normalizedSyncPath.length).split("/").slice(0, -1).join("/");
          if (!oldRelPath.startsWith('/')) oldRelPath = '/' + oldRelPath;

          let newRelPath = normalizedNewPath.substring(normalizedSyncPath.length).split("/").slice(0, -1).join("/");
        ...
        console.error(`Error processing file rename: ${oldPath} -> ${newPath}`, err);
      }
